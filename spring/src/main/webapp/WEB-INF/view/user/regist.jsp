...
<script>
	function formCheck(){
		...
		//아이디 중복체크
		var con = true; //con(continue) 이라는 함수
		$.ajax({
			url:"idcheck.do",
			data:{
				id:$("#id").val()
			},
			async:false, // 모든 프로그램은 위에서 아래로 실행되지만 쓰레드처럼 실행됨 (실행흐름과 별개로 돌아감).. 기본옵션은 true
			method:"get",
			success:function(r) {
				//console.log(r);
				if (r.trim() == '1'){
					...
					$("#id").val('');
					$("#id").focus();
					//return false;  // 만나면 함수 실행중에 중지, 써도 소용이 없음 콜백함수니까 (백에서실행)
					con = false; 
				} 
			}
		});
		if (con == false ) return false; // 변수를 사용하여 함수를 바꿔버림 (변수를 사용하여 상태를 제어)
		if ($("#pwd").val() == ''){
			alert('비밀번호를 입력하세요');
			$("#pwd").focus();
			return false;
		...
		//학교정보체크
		$("input[name='school']").each(function(idx){ // input 태그의 name속성의 값이 school 선택자의 객체를 반복
